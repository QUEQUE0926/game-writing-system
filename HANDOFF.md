...
- 灰烬之国档案已灌：facts 24（volatile 5）+ community 16。单测 7 条 `tests/unit/test_web_facts.py`。
...
## 三、当前状态（2026-09-13 收工：灰烬之国 B 路 52 卡待用户验收，A 路 81 卡已弃）

**没有卡点。正在等用户验收这批 B 路手写+联网核验卡（或指出要改的卡）。**

- **dev 库现有 4 个源**：《灰烬之国》正式版 666 句 + 8月更新 515 句、《命运之手》2325 句、《零秒：时光的归途》1481 句（旧 29 源数据 09-12 已清，这四个都是之后重新导入的）。
- **A 路 81 张机器卡已按用户拍板弃掉**（"走个B路"），exports/cards 下 20260913 后缀文件已被 B 路产出覆盖。卡片原始数据 = `cards-state-20260913.json`（52 卡）。
- **B 路 52 卡已产出**（会话 AI 精读 25 窗手写 → 联网核验 → `craft_cards.py --from-json` 渲染，逐字锁 0 拦截）：产出在 `data/dev/exports/cards/`：
...
- **联网核验（2026-09-13 当日全重核）**：商店页 API（名/开发商发行商/EA日期/价¥68当日无折扣）；新闻 API（最新补丁仍 V0.6.11b 8月25日，此后无新补丁）；**官方成就页三枚彩蛋**：「植物大战僵尸」=塞弗林的水壶、「小心！别照镜子！」=美杜莎之眼、「一个都别想跑！」=打地鼠5000分、「斩尽荆棘」=击败塞弗琳；Gamemag.ru 报道 V0.6.10「Isdra」重做。社区佐证沿用小黑盒 665862 话题缓存（09-08~09-12 帖）。转人工（human_check）：EA期约12个月无出处、难度与角色绑定无官方说明、修改器合规性存疑、德鲁伊事件bug无公告、炮手被削无信号。
- **特殊卡已覆盖**：版本对比卡（8月25更新说明、boss轮换+控制免疫互证）、跨游戏提及卡（植物大战僵尸，官方成就同名互证，登记待补线）、自我反转卡（召唤流认输；纯刷子↔爽死了）。
- **命运之手/零秒**：已筛窗（windows-20260913.jsonl 里有它们的窗口），**未粗判未制卡**。
- **最新 commit**：`24c9f24`（导入防线）。本会话另有：`ee6647d`（报告自动清理）、`eaabbcd`（craft_cards 双轨修复）。

## 四、下一步计划（主线）

1. **用户验收 52 张 B 路卡 + 回填两份 human_check**（逐卡勾 采用/删除，改提交状态）。验收通过 → 改名归档（05 流程）→ `import_cards.py --apply` 写库。
