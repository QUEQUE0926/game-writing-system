...
- **本会话跑通了完整管线并产出 33 张卡**：切集（make_episodes --apply）→ 筛窗（25 窗）→ 粗判（KEEP 17 / MAYBE 8 / DROP 0）→ 精读产卡（AI 直产出 33 卡，逐字锁 0 拦截）。产出在 `data/dev/exports/cards/`：两份按版本拆分的草稿 + 两份按版本拆分的 human_check。
...
3. **用户按 human_check 回填人工结论**（本批 5 条：ASR 变体『学会bug』『峨蚜闪电无一』『原子弹』、口播数值、更新日期25号vs公告16号），通过后卡片改名归档（`material_cards.md` 流程），再写库入库（material_cards 表已建，**写库代码还没有**，走新迁移不许动结构）。
4. **跨游戏关联（2026-09-12 用户拍板，方案已写入 05 文档，待实现）**：①玩家提到其他游戏→立对比卡+登记 game_aliases；②类型相近但玩家没说的→素材卡语义相似匹配（qwen3-embedding 有 33 张真卡后复测）；③**联网联想提前到素材卡阶段**（用户明确修改原规矩：不再是"价值拓展型留写作期"，而是按游戏一轮做同类联想，非逐卡）。等第二个游戏数据进库后实现。
...
34. **联网核验来源规矩（2026-09-12 用户拍板）**：来源必须与游戏强相关或与当前这句话强相关；抓取范围=海内外游戏社区：小黑盒、知乎、小红书、B站、其乐（Keylol）、TapTap、Steam社区、YouTube/Reddit 及海外媒体。来源写具体页面（站名+标题/链接），禁止写"知乎/TapTap攻略"这类模糊合集。核对结果填「核对（联网）」字段（已核实+结果+出处 / 未查到转人工），「联网策略」字段只用四值枚举。证据核验型联网（专有名词/版本/可疑转写）不受价值档限制；价值拓展型（热点/同类参照）留后续调度。
