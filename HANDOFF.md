...
- **工作分支常驻 dev**。dev 分支最新 commit：`b798abe`（跨版本引用自动化）。
- prod 仍是干净空库。
...
40. **写卡数据的标点规矩**：web_log/联网字段里不要自己带"待人工："前缀（human_check 渲染会自动加"待人工核对："，重复前缀用户圈过）。卡内标点用直角引号『』，quote 必须与转录逐字一致（含错别字），预先用 norm+find 自查一遍再送渲染，别浪费逐字锁重试。
