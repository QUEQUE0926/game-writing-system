...
- **B 路·命运之手 26 卡 + 44 骨架已产出（09-13，两段式第二实战）**：粗判 28 窗（15KEEP+13MAYBE）→
  会话 AI 精读全窗 → 两段式手写（26 精写卡全中/高价值：高5/中21 + 44 骨架全窗覆盖）→
  新建 `data/dev/web_facts/命运之手.json`（8 facts+1 community，当日重核）→ 渲染自动挂载 18 条 →
  逐字锁 0 拦截。产出在 `data/dev/exports/cards/`：
  - 《命运之手》素材卡草稿（default）26 卡 + human_check 4 条（前作无信号转人工、等级30/32两处口播不一致、CE修改器合规存疑×2）
  - 《命运之手》素材卡骨架清单（default）44 条（含 ASR 存疑标注：E08『4月24』、E16碎裂赋能疑成功失败颠倒、E26叠字）
  - 卡源 JSON `data/dev/tmp/hof_cards.json`（构建器 `data/dev/tmp/build_hof_cards.py`，cards+skeletons 双锁自检，可复跑）
  - **敏感卡处理**：E01 商单口径卡（原话登记"引导愿望单/不提发行/不拉踩"，发行商已核实但成文不点名）；
    E24 CE 修改器卡（登记事实+隔离数值+合规存疑转人工，中档默认不勾）；E27『这开挂才能过』为比喻义骨架已标注。
  - **职业反转卡组**：E07战士好用→E18战士吃力→E20战士好难换魔术师→E21法师容易（自我反转卡+关联线已成）
